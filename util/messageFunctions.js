...
module.exports = (message, client) => {
    message.embed = () => {
        return new MessageEmbed()
            .setColor(client.config.embedColor || "#0e64ed")
            .setTimestamp()
            .setFooter(`© ${client.config.servername || message.guild && message.guild.name || ""} - ${message.author ? `${client.l._by} ${message.author.username}` : ""}`);
    }
    ...
    if (client.db) {
        message.loadMemberSettings = async function (member) {
            let settings = await client.db.get(`${message.guild.id}-${member.user.id}`) || client.config.settings.user;
            return settings;
        }
        ...
        if (message.guild) {
            ...
            if (!message.guild.settings || !message.guild.settings.prefix) message.guild.settings = client.config.settings.guild;
