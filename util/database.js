module.exports = class Database {
  constructor({development = false, cwd = __dirname, name ="database"} = {development: false, cwd: __dirname, name: "database"}) {
    ...
    let i = setInterval(() => {
      ...
      clearInterval(i);
      this.sqlite.run("CREATE TABLE IF NOT EXISTS framework (key TEXT, value TEXT);", () => {
        this.sqlite.all("SELECT * FROM framework;", (err, data) => {
          if (err) throw err;
          data.forEach(d => {
  ...
  get(key) {
    ...
    return new Promise((res) => {
      this.sqlite.all(`SELECT value FROM framework WHERE key='${key}';`, [], (err, data) => {
        if (err) throw err;
  ...
  set(key, value) {
    ...
    return new Promise((res) => {
      this.sqlite.all(`SELECT * FROM framework where key='${key}'`, [], (err, data) => {
        if (err) throw err;
        let sql = "";
  ...
  delete(key) {
    ...
    return new Promise((res) => {
      this.sqlite.all(`DELETE FROM framework WHERE key='${key}'`, [], (err) => {
        if (err) throw err;
        res(true);
      });
    });
